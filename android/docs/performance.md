...
VaultNote preserves the cold-start sequence: construct a lightweight lazy container, inflate lock/local UI, start bounded Room flows, draw, then run optional attachment migration, OCR, and sync scheduling. Backup crypto, staging, ZIP processing, ML Kit, WorkManager, Coil, OkHttp, LAN discovery, hashing, thumbnails, PDFs, and OCR temporary storage are not initialized from `Application.onCreate()`. OkHttp’s AndroidX Startup initializer is removed from the merged manifest; the pinned client is built only for pairing or background sync.
...
LAN attachment upload/download uses 64 KiB streaming and bounded parallelism from unique WorkManager sync. Device ciphertext is transformed to shared ciphertext without plaintext disk materialization. Resumable downloads retain only bounded app-private ciphertext partials. Item JSON and encrypted item envelopes are capped near 2 MiB; change pages are capped at 200 records and four pages per worker run.
