...
class AndroidBackupRepositoryTest {
    ...
    @Test
    fun `database snapshot reader derives stable order for version one backups`() {
        val sink = CapturingDatabaseSink()
        val snapshot = """
            {
              "schemaVersion":1,
              "itemCount":1,
        ...
        BackupDatabaseCodec(database.backupDao()).read(
            ByteArrayInputStream(snapshot.encodeToByteArray()),
            sink,
        )

        assertEquals(1, sink.items.size)
        assertEquals(Long.MAX_VALUE - 20L, sink.items.single().sortPosition)
    ...
    private class CapturingDatabaseSink : BackupDatabaseSink {
        val items = mutableListOf<VaultItemEntity>()
        var finished = false

        override fun begin(expectedItems: Long, expectedAttachments: Long) {
            assertEquals(1L, expectedItems)
            assertEquals(0L, expectedAttachments)
        }
        ...
        override fun acceptTag(tag: TagEntity) = Unit

        override fun acceptItemTag(reference: ItemTagCrossRef) = Unit

        override fun acceptAttachment(attachment: AttachmentEntity, entryPath: String) = Unit
