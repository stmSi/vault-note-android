...
class VaultViewModelTest {
    ...
    @Test
    fun `reorder forwards stable neighbors and reports completion`() =
        runTest(mainDispatcherRule.dispatcher) {
            val repository = WindowedFakeRepository(itemCount = 3)
            val viewModel = VaultViewModel(repository, SavedStateHandle())
            ...
            viewModel.reorderItem(
                itemId = summary(0).id,
                previousItemId = summary(2).id,
                nextItemId = summary(1).id,
            )
            advanceUntilIdle()

            assertEquals(
                Triple(summary(0).id, summary(2).id, summary(1).id),
                repository.reorderedItems.single(),
            )
            val event = events.single() as VaultEvent.ItemReorderFinished
            assertEquals(true, event.committed)
            assertEquals(null, event.warningOrError)
        }
    ...
    private class WindowedFakeRepository(itemCount: Int) : VaultRepository {
        ...
        val reorderedItems = mutableListOf<Triple<String, String?, String?>>()
        ...
        override suspend fun reorderActiveItem(
            ...
            nextItemId: String?,
        ): RepositoryResult<Unit> = RepositoryResult.Success(Unit).also {
            reorderedItems += Triple(id, previousItemId, nextItemId)
        }
