...
abstract class VaultDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** Adds persistent manual ordering while retaining the pre-migration visible order. */
            ...
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.query(
                    ...
                    while (cursor.moveToNext()) {
                        db.execSQL(
                            "UPDATE vault_items SET sort_position = ? WHERE id = ?",
                            arrayOf(position, cursor.getString(0)),
                        )
                        position += SORT_POSITION_GAP
                    }
                }
                db.execSQL("DROP INDEX IF EXISTS index_vault_items_active_order")
                db.execSQL(
                    """
                    CREATE INDEX IF NOT EXISTS index_vault_items_active_order
                    ON vault_items(
                        deleted_at,
                        is_archived,
                        is_pinned,
