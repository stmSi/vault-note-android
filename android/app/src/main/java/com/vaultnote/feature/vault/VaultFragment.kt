...
class VaultFragment : Fragment() {
    ...
    private val noteAdapter = VaultItemAdapter(
        ...
        onReorder = { id, previousId, nextId ->
            viewModel.reorderItem(id, previousId, nextId)
        },
    )
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        itemTouchHelper = ItemTouchHelper(
            ...
            helper.attachToRecyclerView(currentBinding.noteList)
            noteAdapter.dragStarter = helper::startDrag
        }
    ...
    private fun render(currentBinding: FragmentVaultBinding, state: VaultUiState) {
        ...
        when (state) {
            is VaultUiState.Content -> {
                ...
                noteAdapter.submitVaultList(state.items) {
            ...
            -> noteAdapter.submitVaultList(emptyList())
        }
    ...
    private fun handleEvent(event: VaultEvent) {
        when (event) {
            ...
            is VaultEvent.ItemReorderFinished -> {
                if (event.committed) {
                    noteAdapter.confirmReorder()
                    if (event.warningOrError != null) {
                    ...
                    noteAdapter.rollbackReorder()
                    binding?.root?.let { root ->
                        Snackbar.make(root, R.string.reorder_failed, Snackbar.LENGTH_LONG).show()
                    }
                }
            }
        }
