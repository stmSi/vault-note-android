...
class VaultDatabaseSchemaTest {
    ...
    fun versionOneDataMigratesToCurrentVersion() {
        ...
        migrationHelper.runMigrationsAndValidate(
            ...
            *VaultDatabase.ALL_MIGRATIONS,
            ...
            db.query(
                """
                SELECT title, color, sort_position, local_revision, remote_revision,
                       last_synced_revision, server_version_token, deleted_at
                ...
                assertEquals("server-v5", cursor.getString(6))
                assertEquals(11L, cursor.getLong(7))
            }
            db.query(
                """
    ...
    @Test
    fun versionFourMigrationSeedsCurrentPinnedAndUpdatedOrder() {
        migrationHelper.createDatabase(TEST_DATABASE_NAME, 4).use { db ->
            insertVersionFourItem(db, id = "older", pinned = false, updatedAt = 10L)
            insertVersionFourItem(db, id = "newer", pinned = false, updatedAt = 20L)
            insertVersionFourItem(db, id = "pinned", pinned = true, updatedAt = 5L)
        }
        ...
        migrationHelper.runMigrationsAndValidate(
            ...
            *VaultDatabase.ALL_MIGRATIONS,
            ...
            db.query(
                """
                SELECT id, sort_position FROM vault_items
                ...
                assertEquals(
                    listOf(
                        "pinned" to 0L,
                        "newer" to 1_000_000_000_000L,
                        "older" to 2_000_000_000_000L,
                    ),
                    rows,
                )
            }
    ...
    private fun insertVersionFourItem(
        ...
        db.execSQL(
            ...
            INSERT INTO vault_items (
                ...
                last_synced_revision, server_version_token, sync_status, deleted_at,
                conflict_origin_id
            ) VALUES (?, 'NOTE', 'DEFAULT', ?, '', '', ?, 0, 0, 1, ?, 1, NULL, NULL, NULL,
                      'PENDING', NULL, NULL)
            """.trimIndent(),
            arrayOf<Any>(id, id, if (pinned) 1 else 0, updatedAt),
        )
    }
