...
internal class RestoreStagingStore private constructor(
    ...
    fun readItemsPage(
        ...
        SELECT original_id, final_id, type, color, title, body, ocr_text,
               is_pinned, is_favorite, is_archived, created_at, updated_at,
        ...
        buildList {
            while (cursor.moveToNext()) {
                val mappedConflict = cursor.optionalString(14)?.let(::mappedItemId)
                add(
                    cursor.requiredString(0) to VaultItemEntity(
                        ...
                        localRevision = cursor.getLong(12).coerceAtLeast(1L),
                        remoteRevision = null,
                        lastSyncedRevision = null,
                        serverVersionToken = null,
                        syncStatus = ItemSyncStatus.PENDING,
                        deletedAt = cursor.optionalLong(13),
