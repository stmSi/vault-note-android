...
| Compromised cloud storage | Not yet applicable; future design required | Phase 3 performs no cloud upload. Future storage must receive only authenticated encrypted attachments and must not receive plaintext keys. Metadata confidentiality is not yet designed. |
...
## Security properties tested in Phase 3
...
- Security review of OCR and any future native parser/library before it is introduced.
