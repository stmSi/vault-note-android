...
| Lost encryption keys | Mitigated only with an independent backup | Keystore loss or app-data clearing destroys access to installation ciphertext. A previously exported password-encrypted backup can recover content without the old Keystore key. Without that archive and password, recovery is impossible. |
| Corrupted backups | Substantially mitigated | Strict manifests, ciphertext SHA-256, AES-GCM authentication, exact entry sets, bounded ZIP parsing, content validation and private staging reject damage before live changes. Loss of the only valid archive or its password is not recoverable. |
